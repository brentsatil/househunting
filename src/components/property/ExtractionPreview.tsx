...
import {
  Bed, Bath, Car, MapPin, Check, Pencil, X, Loader2, AlertTriangle,
} from "lucide-react";
...
export function ExtractionPreview({
  ...
  const hasMissing = edited.missing_fields && edited.missing_fields.length > 0;
  ...
  // Auto-confirm if all required fields present and not editing
  const canSave = edited.address && edited.suburb && edited.postcode && edited.state;
    ...
    <div className="rounded-2xl border-2 border-primary/20 bg-primary/[0.02] overflow-hidden animate-in slide-in-from-top-2 duration-300">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-primary/5 border-b border-primary/10">
        <span className="text-xs font-medium text-primary">
          {hasMissing ? "Almost there — fill in missing details" : "New listing extracted"}
        </span>
        ...
        <div className="p-4 space-y-3">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                >
                  <SelectTrigger className={`mt-1 h-9 ${!edited.state ? "border-amber-400" : ""}`}>
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => updateField("parking", e.target.value ? Number(e.target.value) : undefined)}
                  className="mt-1 h-9"
                />
              </div>
            </div>
            <div>
              <Label className="text-xs">{mode === "rent" ? "Rent ($/wk)" : "Price ($)"}</Label>
              <Input
                type="number"
                value={
                  mode === "rent"
                    ? edited.rent_weekly ? edited.rent_weekly / 100 : ""
                    : edited.sale_price ? edited.sale_price / 100 : ""
                }
                onChange={(e) => {
                  const val = e.target.value ? Number(e.target.value) * 100 : undefined;
                  updateField(mode === "rent" ? "rent_weekly" : "sale_price", val);
                }}
                className="mt-1 h-9"
              />
            </div>
          ...
          <div className="flex gap-2 pt-1">
            <Button
              ...
              onClick={() => {
                setEditMode(false);
                // Clear missing fields for ones we've filled
                setEdited((prev) => ({
                  ...prev,
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            ...
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              {edited.bedrooms != null && (
                <span className="flex items-center gap-1"><Bed className="h-3.5 w-3.5" />{edited.bedrooms}</span>
              )}
              {edited.bathrooms != null && (
                <span className="flex items-center gap-1"><Bath className="h-3.5 w-3.5" />{edited.bathrooms}</span>
              )}
              {edited.parking != null && (
                <span className="flex items-center gap-1"><Car className="h-3.5 w-3.5" />{edited.parking}</span>
              )}
