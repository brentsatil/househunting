...
export default function PropertyDetailPage() {
  ...
  const partnerId = partnership && userId
    ? userId === partnership.user1_id ? partnership.user2_id : partnership.user1_id
    : null;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-6 max-w-6xl mx-auto space-y-5">
        ...
        <PropertyDetail
          property={property} interactions={propertyInteractions}
          mode={mode} currentUserId={userId}
          onRatingChange={async (rating) => { await updateInteraction(propertyId, userId, { rating }); }}
          onStatusChange={async (status) => { await updateProperty(propertyId, { status }); }}
        />

        {/* Tabbed sections */}
        <Tabs defaultValue="plan" className="w-full">
          <TabsList className="w-full grid grid-cols-4 h-12 rounded-xl bg-secondary p-1">
            <TabsTrigger value="plan" className="rounded-lg text-sm data-[state=active]:shadow-sm">
              Plan
            </TabsTrigger>
            <TabsTrigger value="discuss" className="rounded-lg text-sm data-[state=active]:shadow-sm">
              Discuss
              {comments.length > 0 && (
            ...
            <TabsTrigger value="rate" className="rounded-lg text-sm data-[state=active]:shadow-sm">
              Rate
            </TabsTrigger>
            <TabsTrigger value="area" className="rounded-lg text-sm data-[state=active]:shadow-sm">
              Area
            </TabsTrigger>
          </TabsList>

          <TabsContent value="plan" className="mt-4">
            <InspectionTracker propertyId={propertyId} userId={userId}
              partnerId={partnerId} partnerName={partnerName} address={property.address} />
          </TabsContent>
          <TabsContent value="discuss" className="mt-4">
            <CommentThread comments={comments} currentUserId={userId}
              onAddComment={(content) => addComment(content, userId)} />
          </TabsContent>
          <TabsContent value="rate" className="mt-4">
            <PartnerRatings interactions={propertyInteractions} currentUserId={userId}
              onRatingChange={async (rating) => { await updateInteraction(propertyId, userId, { rating }); }}
              user1Name="You" user2Name={partnerName} />
          </TabsContent>
